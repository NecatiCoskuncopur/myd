...
const SideMenu = ({ role, open, toggleDrawer }: Props) => {
  ...
  const signOutItem: UserTypes.ISidebarItem = {
    key: 'signout',
    label: 'Çıkış Yap',
    icon: <Logout />,
  ...
  const renderItems = (items: UserTypes.ISidebarItem[], depth = 0): React.ReactNode =>
    items.map(item => {
        ...
        return (
          <React.Fragment key={item.key}>
            <ListItemButton onClick={() => handleToggle(item.key)} sx={{ pl: 2 + depth * 2 }}>
              ...
              {showText && <ListItemText primary={<Typography sx={{ fontWeight: 500, letterSpacing: '0.02em' }}>{item.label}</Typography>} />}
