...
const EditUser = ({ open, onClose, user, onSuccess }: Props) => {
  ...
  const [pricingLists, setPricingLists] = useState<IPricingList[]>([]);
    ...
    const fetchPricingLists = async () => {
      try {
        ...
        if (result.status === 'OK' && result.data) {
          ...
          setErrorMessage(result.message ?? GENERAL.UNEXPECTED_ERROR);
        }
      } catch (error) {
  ...
  const onSubmit = (values: ISetUserPayload) => {
    setErrorMessage(null);
    ...
    startTransition(async () => {
      try {
        ...
        if (response.status === 'ERROR') {
          setErrorMessage(response.message ?? GENERAL.UNEXPECTED_ERROR);
          return;
        }

        setSnackbar({
          open: true,
          message: response.message ?? USER.SUCCESS,
          severity: 'success',
        });
        onSuccess?.();
        reset();
