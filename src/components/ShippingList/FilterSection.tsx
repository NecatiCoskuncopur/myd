...
const FilterSection = ({ searchParams }: FilterSectionProps) => {
  ...
  return (
    ...
    >
      <TextField label="Alıcı Adı" size="small" value={inputs.consigneeName} onChange={e => setInputs(prev => ({ ...prev, consigneeName: e.target.value }))} />
      <TextField
        label="Alıcı Telefon"
        size="small"
      ...
      />
      <TextField label="Takip No" size="small" value={inputs.trackingNumber} onChange={e => setInputs(prev => ({ ...prev, trackingNumber: e.target.value }))} />
      <DatePicker
        label="Başlangıç"
      ...
      <StyledButton type="submit" variant="contained" startIcon={<SearchIcon />} sx={{ height: 40 }}>
        Ara
      </StyledButton>

      {isFiltered && (
        <Button type="button" variant="text" color="inherit" onClick={handleReset} sx={{ height: 40 }}>
          Temizle
        </Button>
      )}
      ...
      <Button
        ...
        sx={{ height: 40, fontSize: 12, ml: 'auto' }}
      >
