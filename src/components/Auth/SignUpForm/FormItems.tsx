...
const { ADDRESS, CAPTCHA, PASSWORD, USER } = messages;
const { CITY, DISTRICT, LINE, POSTALCODE } = ADDRESS;
const { COMPANY, EMAIL, FIRSTNAME, LASTNAME, PHONE } = USER;

type FormItemsProps = {
  errors: FieldErrors<ISignUpPayload>;
  control: Control<ISignUpPayload, ISignUpPayload>;
  setValue: UseFormSetValue<ISignUpPayload>;
  captchaKey: number;
};
